 ...
 * 
 * WinCrypt32 클래스
 * 출처: http://stackoverflow.com/questions/3281057/get-timestamp-from-authenticode-signed-files-in-net
 * 
 */
...
{
    /// <summary>
    /// crypt32.dll을 마샬링하여 타임스탬프, 연대 서명 등의 정보를 제공해줍니다.
    /// </summary>
    static class WinCrypt32
    {
        public static bool IsTimestamped(string filePath)
        {
            try
            {
                int encodingType;
                ...
                foreach (var signerInfo in signedCms.SignerInfos)
                {
                    foreach (var unsignedAttribute in signerInfo.UnsignedAttributes)
                    {
                        ...
                        foreach (var counterSignInfo in signerInfo.CounterSignerInfos)
                        {
                            ...
                            {
                                if (signedAttribute.Oid.Value != szOID_RSA_signingTime) continue;
